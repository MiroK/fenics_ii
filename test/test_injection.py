...
W = [U, Lc]
u, lmc = map(TrialFunction, W)
v, gmc = map(TestFunction, W)
...
left_expr = Expression(map(sp.printing.ccode, left), degree=1)

u_expr = Expression(map(sp.printing.ccode, u), degree=1)
g_expr = Expression(map(sp.printing.ccode, g), degree=1)
