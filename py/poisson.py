...
g = f
u_exact = Expression('pi*pi*U', U=f, degree=4)


def main(n):
    '''
    Minimize 0.5*inner(grad(u), grad(u))*dx-inner(f, v) over 
    ...
    bcV = DirichletBC(V, Constant(0), 'on_boundary')
    bcs = block_bc([[bcV], None], True)
...
if __name__ == '__main__':
    ...
    for n in [2, 4, 8, 16, 32, 64, 128]:
        size, niters, uh, ph, h = main(n)
        if rate < 0:
            e = errornorm(u_exact, uh, 'H1')
            rate = ln(e/e0)/ln(h/h0)
        e0, h0 = e, h

        msg = 'size = %s, niters=%d, e(rate)=%.4E(%.2f)' % (size, niters, e0, rate)
