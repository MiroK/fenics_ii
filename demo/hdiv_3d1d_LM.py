...
# Multiplie space
Y = FunctionSpace(bmesh, 'DG', 0)

W = (V, Vb, Q, Qb, Y)

u, ub, p, pb, x = map(TrialFunction, W)
v, vb, q, qb, y = map(TestFunction, W)
...
# Add cooupling
A[0][4] += ii_assemble(inner(x, Pi_v)*dx_)
A[1][4] += ii_assemble(-inner(x, vb)*dx_)
A[4][0] += ii_assemble(inner(y, Pi_u)*dx_)
A[4][1] += ii_assemble(-inner(y, ub)*dx_)

wh = ii_Function(W)
solve(monolithic(A), wh.vector(), monolithic(b))

uh, ubh, ph, pbh, lmh = wh
wh.rename([('uh', ''), ('ubh', ''), ('ph', ''), ('pbh', ''), ('lmh', '')])
