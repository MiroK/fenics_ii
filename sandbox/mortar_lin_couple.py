...
def mortar_lin_couple(N):
    ...
    F = [inner(nl1(u1)*grad(u1), grad(v1))*dx + inner(u1, v1)*dx + inner(p, Tv1)*dxGamma - inner(f1, v1)*dx,
         ...
         inner(Tu1, q)*dxGamma - inner(Tu2, q)*dxGamma - inner(h, q)*dxGamma]
...
if __name__ == '__main__':
    ...
    File('./nl_results/mortar_lin_u1h.pvd') << u1h
    File('./nl_results/mortar_lin_u2h.pvd') << u2h
    File('./nl_results/mortar_lin_ph.pvd') << ph
